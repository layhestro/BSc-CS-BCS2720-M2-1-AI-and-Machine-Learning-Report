...
\subsection{Research Question 3: Resource Usage Prediction}
...
\begin{itemize}
    ...
    \item \texttt{num\_parallel\_agents}: Integer (number of parallel environment instances)
\end{itemize}
...
\textbf{ML Approach:} Memory usage may exhibit more linear relationships as it often scales predictably with certain parameters (buffer size, network size). We start with Linear Regression as baseline and compare with Random Forest, Extra Trees, and KNN. We also test HistGradient Boosting and MLPRegressor for potential non-linear effects.
